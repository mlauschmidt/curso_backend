...
socket.on('producto_agregado_inventario', (data) => {
    ...
    `<div id="prod-${product.id}" style="border-bottom: 1px solid black">   
        ...
        <button style="margin: 0px 0px 20px 20px" class="deleteProdList" id="deleteProdList_${product.id}" onclick="deleteProdList('${product.id}')">Eliminar</button>
        <button style="margin: 0px 0px 20px 20px" class="updateProdButton" id="updateProdButton_${product.id}" onclick="updateProdButton('${product.id}')">Editar</button>
    </div>`;
})
    ...
    .then (product => {
        ...
        `<ul>
            <li>Nombre: <input id="newProductTitle" type="text" name="title" value="${product.title}">
                <ul>
                    <li>Id: <input id="newProductId" type="text" name="id" value="${product.id}" readonly></li>
