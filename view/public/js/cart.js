...
socket.on('producto_agregado_carrito', (data) => {
    ...
    const updatedProduct = document.getElementById(`prod-${product.product.id}`);
    ...
    if (updatedProduct){
        ...
        <td><button style="margin: 10px" class="deleteProdCart" id="deleteProdCart_${product.product.id}" onclick="deleteProdCart('${product.cartId}','${product.product.id}')">Eliminar</button></td>`;
    } else {
        cartContainer.innerHTML += 
        `<tr id="prod-${product.product.id}">
            <td>${product.product.title}</td>
            <td>${product.product.price}</td>
            <td>${product.quantity}</td>
            <td><button style="margin: 10px" class="deleteProdCart" id="deleteProdCart_${product.product.id}" onclick="deleteProdCart('${product.cartId}','${product.product.id}')">Eliminar</button></td>
